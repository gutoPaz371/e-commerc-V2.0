...
function findImage($id) {
  ...
  foreach ($extensions as $ext) {
      $file = "../vendedor/uploads/produtos/fts/{$id}/0.{$ext}";
      if (file_exists($file)) {
          return $file;
      }
  }

  return null; // Retornar null se nenhum arquivo for encontrado
}
?>
<!DOCTYPE html>
              ...
              <div class="row flex-between-center">
                ...
                <div class="col-sm-auto">
                  <div class="row gx-2 align-items-center">
                    ...
                    <div class="col-auto pe-0" hidden> <a class="text-600 px-1" href="../app/e-commerce/product/product-list.html" data-bs-toggle="tooltip" data-bs-placement="top" title="Product List"><span class="fas fa-list-ul"></span></a></div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card">
            <div class="card-body">
              ...
              <?php
                ...
                $prod = new Produto($db);
                $res = $prod->selectAllAtive();
                while($row=$res->fetch_array()){
              ?>
                <div class="mb-4 col-md-6 col-lg-4">
                  <div class="border rounded-1 h-100 d-flex flex-column justify-content-between pb-3">
                    <div class="overflow-hidden">
                      <div class="position-relative rounded-top overflow-hidden"><a class="d-block" href="#aqui"><img class="img-fluid rounded-top" src="<?php echo findImage($row['id']); ?>" alt="" /></a><span class="badge rounded-pill bg-success position-absolute mt-2 me-2 z-2 top-0 end-0" hidden>New</span>
                      </div>
                      <div class="p-3">
                        <h5 class="fs-9"><a class="text-1100" href="<?php echo "./product-details.php?id=".$row['id']."&token=".md5(md5($row['id']).md5($row['id'])); ?>"><?php echo $row['nome']; ?></a></h5>
                        <p class="fs-10 mb-3"><a class="text-500" href="#!" hidden>Computer &amp; Accessories</a></p>
                        <h5 class="fs-md-7 text-warning mb-0 d-flex align-items-center mb-3">R$ <?php echo $row['valor'] ?>
                          <del class="ms-2 fs-10 text-500" hidden>$2399 </del>
                        </h5>
                        <p class="fs-10 mb-1" hidden>Shipping Cost: <strong>$50</strong></p>
                        <p class="fs-10 mb-1" hidden>Stock: <strong class="text-success">Available</strong>
                        </p>
                      </div>
                    </div>
